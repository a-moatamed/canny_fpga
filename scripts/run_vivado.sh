...
HLS_IP_REPO="${HLS_IP_REPO:-${LAB_DIR}/hls/build/export}"
BOARD_PART="${BOARD_PART:-}"
DEFAULT_HLS_IP_ZIP="${LAB_DIR}/hls/build/export.zip"
UNPACK_DIR="${LAB_DIR}/hls/build/export_ip"
...
if [[ -d "${HLS_IP_REPO}" ]]; then
...
elif [[ "${HLS_IP_REPO}" == "${LAB_DIR}/hls/build/export" && -f "${DEFAULT_HLS_IP_ZIP}" ]]; then
  if ! command -v unzip >/dev/null 2>&1; then
  ...
  echo "  - directory: ${LAB_DIR}/hls/build/export"
  echo "  - archive:   ${LAB_DIR}/hls/build/export.zip"
...
if [[ -n "${BOARD_PART}" ]]; then
  vivado -mode batch -source "${LAB_DIR}/vivado/build_overlay.tcl" \
